...
目前版本：**1.1.0**。驗證範圍與限制見 [驗證紀錄](VALIDATION.md)。
...
| 聲音調整 | 電腦與麥克風分軌開關、音量及整段淡入淡出 |
| 成品匯出 | MP3 192／320 kbps，或 48 kHz / 16-bit WAV |

音訊擷取與存檔在本機完成，程式沒有音訊上傳功能。

![EchoReplay 音檔庫與剪輯介面，使用合成示範音訊](docs/images/editor.png)
...
- 剪輯器每次支援最多 15 分鐘的單聲道／立體聲音檔。試聽也會被電腦錄音收錄；必要時可自行暫停錄音。
- MP3 使用 Windows Media Foundation；Windows N／精簡系統可能需要安裝媒體功能，WAV 剪輯與匯出不依賴 MP3 編碼器。
- 目前尚未包含 Discord／遊戲各自分軌、AI 人聲分離或降噪。
