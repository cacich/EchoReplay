...
EchoReplay 是 Windows 的純音訊即時回放與剪輯工具。背景持續保留最近幾分鐘的電腦聲音與麥克風，在有趣的事情發生後，按快捷鍵保存，再直接選取最後幾秒、剪輯並輸出 MP3，省去另外開啟剪輯軟體的步驟。

目前版本：**1.2.0**。新增語音／遊戲分軌與排除試聽的錄音模式。驗證範圍與限制見 [驗證紀錄](VALIDATION.md)。
...
| 程式分軌 | 指定 Discord 等語音程式及遊戲，各自擷取程序樹，另錄麥克風 |
| 排除試聽 | 支援所有程式或指定程式擷取，排除本次 EchoReplay 的試聽音訊 |
| 程式重連 | 記住 .exe 名稱，來源關閉時等待，重啟後自動重新連接 |
...
| 音檔庫 | 搜尋、匯入 WAV／MP3 副本、改名及移至資源回收筒 |
| 波形剪輯 | 拖曳選取、滾輪縮放、精確秒數、快速選最後 5／10／15／30 秒 |
| 非破壞式編輯 | 裁切、多段刪除並接合、區間靜音、復原／重做、保存進度 |
| 聲音調整 | 語音／電腦、遊戲與麥克風分軌開關、音量及整段淡入淡出 |
| 成品匯出 | MP3 192／320 kbps，或 48 kHz / 16-bit WAV |

音訊擷取與存檔在本機完成，程式沒有音訊上傳功能。

![EchoReplay 三軌剪輯介面，使用合成示範音訊](docs/images/three-track-editor.png)
...
1. 開啟 Discord 與遊戲，在「設定」選擇「語音聊天／遊戲分軌」，指定兩個程式並套用。麥克風仍從裝置選單選擇。
2. 播放聲音並說話，確認音量條有反應。
3. 發生有趣的事後按 **Ctrl + Alt + F9**，儲存當下之前的音訊。
4. 按 **Ctrl + Alt + F10** 顯示／隱藏介面。
5. 點「剪輯／試聽最新錄音」，選最後 5／10 秒、按「保留選取範圍」，再匯出 MP3。
6. 不再需要原檔時，在音檔庫選取「原始錄音」並按「刪除」。成品預設存入獨立的 `Exports` 資料夾。
7. 在系統匣右鍵選單選「結束 EchoReplay」，才能完全結束程式。

預設保存到使用者「音樂」資料夾的 `EchoReplay` 子資料夾。每次儲存建立獨立資料夾，包含 `混音.wav`；程式分軌模式固定保存 `語音聊天.wav`，並依設定保存 `遊戲.wav`、`麥克風.wav`。剪輯器可關閉遊戲音軌，只匯出對話。

**升級不會改動原有錄音模式。** 請到設定切換新模式；只想排除試聽、仍錄全部程式，可選「所有程式（排除 EchoReplay 試聽）」。一般 Windows 10 使用「指定播放裝置」相容模式。
...
- 程式模式需要 Windows 11 或 Windows build 20348 以上，涵蓋所選程序樹在不同播放裝置上的聲音。一般 Windows 10 22H2（19045）請使用相容的指定裝置模式；此模式仍會錄進試聽。
- 麥克風直接從 Windows 擷取，**Discord 的靜音、按鍵發話與降噪設定不會套用到這份錄音**。
- 裝置斷線會重試，但無法補回斷線或電腦睡眠期間的聲音。
- 剪輯器每次支援最多 15 分鐘的單聲道／立體聲音檔。新模式會排除本次 EchoReplay 的播放，但喇叭聲仍可能被麥克風實體收錄。
- MP3 使用 Windows Media Foundation；Windows N／精簡系統可能需要安裝媒體功能，WAV 剪輯與匯出不依賴 MP3 編碼器。
- 程式分軌只適用之後的錄音。舊混音不能還原來源；尚未包含 AI 人聲分離或降噪。同一 Discord 內的朋友仍在同一軌。
